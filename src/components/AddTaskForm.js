...
export default function AddTaskForm({isPopupActive,setIsPopupActive}) {
    ...
    return (
        <div className={`formPopup transition-all duration-200 ${isPopupActive ? 'block w-full' : 'hidden w-0'}  py-7 px-7 lg:px-28 h-screen bg-tra absolute top-0 left-0 z-10`}>
            <form className='w-full bg-white h-full py-14 flex justify-center items-center flex-col relative' onSubmit={handleSubmit(onSubmit)}>
                ...
                <h1 className='text-2xl font-semibold text-center text-primary mb-3'>Add Task</h1>
                <div className='flex flex-col h-full w-5/6 lg:w-1/2'>
                    ...
                    />
                    <label className='text-xs mt-1 lg:mt-3 text-gray-400 ' htmlFor="description">Description</label>
                    <textarea
                        id='description'
                    ...
                    <input
                        id='due_date'
                        className='rounded-lg bg-silver h-10 w-full my-2 text-black placeholder:text-gray-600 px-5'
                        type="date" placeholder="due_date" {...register("due_date", { required: true })}
                    />
                    <span>

                    </span>
                    <label className='text-xs mt-1 lg:mt-3 text-gray-400 ' htmlFor="duration">Duration</label>
                    <input
                        id='duration'
                        className='rounded-lg bg-silver h-10 w-full my-2 text-black placeholder:text-gray-600 px-5'
